          ...
          <div className="lg:w-1/4">
            <div className="bg-gradient-to-br from-gray-900 to-black p-6 rounded-2xl border border-red-800/30 sticky top-24">
              <h3 className="font-bold text-red-500 mb-6 text-xl flex items-center">
                ...
                FILTROS
                {getSelectedCategoriesCount() + selectedBrands.length > 0 && (
                  <Badge className="ml-2 bg-red-600 text-white">
                    {getSelectedCategoriesCount() + selectedBrands.length}
                  </Badge>
                )}
              </h3>
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {brands.map((brand) => (
                    <div key={brand.id} className="flex items-center space-x-2">
                      ...
                      <label
                        htmlFor={brand.id}
                        className="text-sm text-gray-300 cursor-pointer flex-1 flex justify-between hover:text-red-400 transition-colors"
                      >
                        <span>{brand.name}</span>
                        <span className="text-red-500">({brand.products_count})</span>
                      </label>
                    </div>
                  ))}
                </div>
